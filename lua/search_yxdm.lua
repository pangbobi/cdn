...
script_info = {
	...
	["version"] = "0.0.2",
	["color"] = "#800080",
	["description"] = "官网：http://www.yxdm.org"
}
...
function onInitAnime()
	local data = request("http://www.yxdm.org/")
	local anime_week = {}
	...
	for i = 1, 7 do
		...
		while tmp do
			...
			table.insert(anime_day, {["url"] = "http://www.yxdm.org/getdlist.php?id="..id, ["name"] = name})
			begin = b + 1
		end
...
function onSearch(keyword, page)
	local data = request("http://www.yxdm.org/search.html?keyword="..pd.urlEncode(keyword).."&searchtype=titlekeyword&channeltype=0&orderby=&kwtype=0&pagesize=18&typeid=0&PageNo="..page)
	local result = {}
	local start = 1
	while true do
		...
		if #img > 0 and string.byte(img) == 47 then
			img = "http://www.yxdm.org"..img
		end
		table.insert(result, {["url"] = "http://www.yxdm.org/getdlist.php?id="..id, ["title"] = pd.htmlUnescape(title), ["image"] = img, ["icon_size"] = "50,67", ["time"] = time, ["description"] = "更新至："..desc})
		start = b + 1
	end
	return result
end
