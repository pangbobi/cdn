...
function onInitTask(task, user, file)
	...
	if j.code==220 then
		local url=requesturl.."method=request&code="..codekey.."&data="..pd.base64Encode(string.gsub(string.gsub(file.dlink, "https://d.pcs.baidu.com/file/", "&path="), "?fid", "&fid"))
		local data = ""
		local c = curl.easy{
			url = url,
			followlocation = 1,
		...
		if j.code==200 then
			...
			task:setIcon("icon/accelerate.png", "私人账号极速下载中")
			return true
		else
